...
countMatrixModel2 <- setRefClass(
  ...
  methods = list(
    ...
    initialize = function(nmax_, 
      ...
      lambda0_factor     <- lambda0_states[,1]
      ...
      lambda1_factor     <- lambda1_states[,2]
      ...
      # drop badness
      gamma01_states <- gamma01_states[gamma01_states[,2] >= 0,]
      
      # get indexes and factors
      gamma01_index      <- stateToReducedIndex(nmax, gamma01_states)
      gamma01_factor     <- gamma01_states[,1]
      ...
      # drop badness
      gamma10_states <- gamma10_states[gamma10_states[,1] >= 0,]
      
      # get indexes and factors
      gamma10_index      <- stateToReducedIndex(nmax, gamma10_states)
      gamma10_factor     <- gamma10_states[,2]
    ...
    solve = function(p, t, method = "ode45", atol = 1e-16, rtol = 1e-16, backward = FALSE, ...) {
